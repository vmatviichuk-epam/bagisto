...
@switch($type)
    ...
    @case('password')
        <v-field
            ...
            {{ $attributes->only(['value', ':value', 'v-model', 'rules', ':rules', 'label', ':label']) }}
        >
            <x-installer::form.control-group.password-visibility>
                <input
                    type="password"
                    name="{{ $name }}"
                    v-bind="field"
                    :class="[errors['{{ $name }}'] ? 'border border-red-600 hover:border-red-600' : '']"
                    {{ $attributes->except(['value', ':value', 'v-model', 'rules', ':rules', 'label', ':label'])->merge(['class' => 'w-full appearance-none rounded-md border px-3 py-2 text-sm text-gray-600 transition-all hover:border-gray-400 ltr:pr-10 rtl:pl-10']) }}
                >
            </x-installer::form.control-group.password-visibility>
        </v-field>
